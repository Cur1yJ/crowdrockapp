...
class Auth_Other extends Crowdrock_Controller {
    ...
    function fb_signin() {
        ...
        if (isset($fb_user)) {
            ...
            $fb = ThirdpartyAuthentication::find('first', array('conditions' => array('facebook_id = ?', $fb_user['id']), 'include' => array('user')));
            ...
            if ($fb && $fb->user) {
                ...
                $user->create_default_associations();
                $fb_data = array(
                    "facebook_id" => $fb_user['id'],
                    "user_id" => $user->id,
                );
                $fb = ThirdpartyAuthentication::create($fb_data);
            }

            if ($fb && $user) {
                if ($this->ion_auth->login($user->email, NULL, FALSE, TRUE)) { //if the login is successful
    ...
    function twitter_signin() {
        ...
        if (!$this->tweet->logged_in()) {
            ...
            if (isset($twitter_user)) {
                ...
                $twitter = ThirdpartyAuthentication::find('first', array('conditions' => array('twitter_id = ?', $twitter_id), 'include' => array('user')));

                if ($twitter && $twitter->user) {
                    $user = $twitter->user;
                } else {
                    $user_data = array(
                        "username" => $twitter_user->id,
                        "email" => isset($twitter_user->email) ? $twitter_user->email : $twitter_user->id . "@twitter.com",
                    ...
                    $twitter = ThirdpartyAuthentication::create($twitter_data);
                }
                ...
                echo 'cannot find the Twitter user';
            }
        }
    }
